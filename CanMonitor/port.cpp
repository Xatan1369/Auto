...
void Port :: ConnectPort(void){//
    ...
    if (thisPort.open(QIODevice::ReadWrite)) {
        ...
        {
            if (thisPort.isOpen()){
                infoPort((SettingsPort.name+ " >> Открыт!\r").toLocal8Bit());
            }
        } else {
            thisPort.close();
            infoPort(thisPort.errorString().toLocal8Bit());
          }
    } else {
        thisPort.close();
        infoPort(thisPort.errorString().toLocal8Bit());
...
void  Port::DisconnectPort(){
    if(thisPort.isOpen()){
        thisPort.close();
        infoPort(SettingsPort.name.toLocal8Bit() + " >> Закрыт!\r");
    }
}
//ot tuta kovuratji!!!
